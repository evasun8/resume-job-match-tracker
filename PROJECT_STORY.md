...
2. **Containerization.** A Dockerfile per service (frontend, backend) plus a `docker-compose.yml` to run them together locally. The concrete technical wrinkle I'm anticipating: Playwright's Chromium dependency needs `--with-deps` when installed inside a slim Docker base image, or it silently fails at runtime.
